...
if [[ $# -ne 4 ]]; then
  echo "Usage: bash server/run_digit_writing_original_protocol3_corner_settle_medium_five_digit_parallel.sh REPO CONFIG GATE1_SUMMARY EXPECTED_HEAD" >&2
  exit 2
fi
...
case "$CONFIG" in
  "$REPO"/configurations/digit_writing_original_protocol3_corner_settle_scale2p50_ref100.json) ;;
  *) echo "ABORT: config is not the checked-in medium five-digit corner-settle config" >&2; exit 1 ;;
esac
...
assert config["variant"] == "corner_settle_medium_five_digit_review6000"
assert config["selected_reference_steps"] == 100
assert config["geometry_config"].endswith("scale2p50_ref100.json")
assert [case["label"] for case in config["cases"]] == expected_cases
...
case "$RUN_ROOT" in
  "$REPO"/runs/digit_writing_original_protocol3_corner_settle_gate/scale2p50/ref100/five_digit_review6000) ;;
  *) echo "ABORT: medium five-digit corner-settle output identity is invalid" >&2; exit 1 ;;
esac
...
if grep -q 'skipped=' "$TEST_LOG"; then
  echo 'ABORT: the medium five-digit server test set contains skipped tests.' >&2
  exit 1
fi
...
CASES=(digit4_medium_baseline digit4_medium_settle100ms digit7_medium_baseline digit7_medium_settle100ms digit2_medium_baseline digit2_medium_settle100ms digit5_medium_baseline digit5_medium_settle100ms digit3_medium_baseline digit3_medium_settle100ms)
PIDS=()
...
if [[ "$FAILED" -ne 0 ]]; then
  echo "ABORT: one or more medium five-digit cases failed; outputs are preserved at $RUN_ROOT" >&2
  exit 1
fi
...
import sys
summary = json.load(open(sys.argv[1], encoding="utf-8"))
assert summary["variant"] == "corner_settle_medium_five_digit_review6000"
assert summary["selected_reference_steps"] == 100
...
echo 'MEDIUM_FIVE_DIGIT_CASES=10'
