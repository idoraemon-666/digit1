...
if [[ $# -ne 6 ]]; then
  echo "Usage: PROTOCOL3_CORNER_EASE_LR_CONTINUATION_AUTHORIZED=1 bash server/run_digit_writing_original_protocol3_corner_ease_lr_continuation_parallel.sh REPO CONFIG SOURCE_RUN_ROOT SOURCE_EVIDENCE_ROOT EXPECTED_HEAD EXPECTED_SOURCE_HEAD" >&2
  exit 2
fi
if [[ "${PROTOCOL3_CORNER_EASE_LR_CONTINUATION_AUTHORIZED:-0}" != "1" ]]; then
  echo 'ABORT: explicit Protocol3 corner-ease LR-continuation approval is required' >&2
  exit 1
fi
...
case "$CONFIG" in
  "$REPO"/configurations/digit_writing_original_protocol3_corner_ease_lr_continuation_6000_to8000.json) ;;
  *) echo 'ABORT: config is not the checked-in corner-ease continuation config' >&2; exit 1 ;;
esac
...
assert config["run_kind"] == "protocol3_corner_ease_lr_continuation"
assert config["variant"] == "six_digit_lr_control_from6000_to8000"
assert config["source"]["repository_head"] == "ee5a1900a33f8a8b7921fd9fc5b09aeed6600925"
assert config["source"]["completed_updates"] == 6000
assert [case["digit"] for case in config["source"]["cases"]] == [0, 3, 4, 5, 6, 7]
...
assert config["decision"]["automatic_winner_selection"] is False
assert config["decision"]["automatic_further_continuation"] is False
assert config["decision"]["automatic_second_seed"] is False
assert config["decision"]["digit8_included"] is False
...
if (( AVAILABLE_CPUS < 12 )); then
  echo 'ABORT: twelve-way parallel execution requires nproc >= 12' >&2
  exit 1
fi
...
CASE_LABELS=(
  digit0_corner_ease digit3_corner_ease digit4_corner_ease
  digit5_corner_ease digit6_corner_ease digit7_corner_ease
)
DIGITS=(0 3 4 5 6 7)
ARMS=(lr1e3 lr3e4)
PIDS=()
...
if [[ "$FAILED" -ne 0 ]]; then
  echo "ABORT: one or more continuation arms failed; outputs are preserved at $RUN_ROOT" >&2
  exit 1
fi
...
"$PYTHON" - "$RUN_ROOT/corner_ease_lr_continuation_summary.json" <<'PY'
import json
import sys
...
assert summary["digit8_included"] is False
