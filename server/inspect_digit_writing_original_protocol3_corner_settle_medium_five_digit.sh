...
if [[ $# -ne 1 ]]; then
  echo "Usage: bash server/inspect_digit_writing_original_protocol3_corner_settle_medium_five_digit.sh RUN_ROOT" >&2
  exit 2
fi
...
case "$RUN_ROOT" in
  ...
  *) echo "ABORT: run root is outside the medium five-digit corner-settle namespace" >&2; exit 1 ;;
esac

echo "TIME=$(date '+%F %T')"
echo '--- PROCESSES ---'
pgrep -af '[p]rotocol3_corner_settle run-case' || true
echo '--- CASE PROGRESS ---'
